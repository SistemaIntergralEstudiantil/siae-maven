...
<html lang="es">
    ...
    <body style="background-image: url(<%=Url.IMG_MENU_PRINCIPAL%>);background-size: 100% 150%;background-position: 0% -35%;background-repeat: no-repeat">
        ...
        <ul class="content-G content">
            <li><a class="content-item-G content-item" href="<%=Url.SER_REPORTE_ASIGNATURA%>">Asignaturas</a></li>
            <li><a class="content-item-G content-item" href="<%=Url.URL_AREAS_MENU%>">Áreas de apoyo</a></li>
            <% if(user != null && user.getRol().endsWith("A") ) { %>
            <li><a class="content-item-G content-item" href="<%=Url.SER_VERANO%>?clave=consulta">Curso de verano</a></li><% } %>
            <li><a class="content-item-G content-item" href="<%=Url.URL_CURRICULA_MENU%>">Currícula</a></li>
            <% if(user != null && user.getRol().endsWith("A") ) { %>
            <li><a class="content-item-G content-item" href="<%=Url.SER_CONTROL_CARGA_ACADEMICA%>?clave=consulta">Altas y bajas</a></li><% } %>
            <li><a class="content-item-G content-item" href="<%=Url.URL_AJUSTES%>">Ajustes</a></li>
            <% if(user != null && user.getRol().endsWith("G") ) { %>
            <li><a class="content-item-G content-item" href="<%=Url.SER_CONTROL%>?clave=course">Control</a></li>
            <li><a class="content-item-G content-item" href="<%=Url.SER_CONTROL_ALTA_BAJA%>?clave=consulta">Academia</a></li><% } %>
            <% if(user != null && user.getRol().endsWith("R") && user.getSemestre() == -2 ) { %>
            <li><a class="content-item-G content-item" href="<%=Url.SER_DOCENTE%>?clave=inicio">Estatus de alumnos</a></li><% } %>
        </ul>
        </nav>
        </header>
        ...
        <form action="<%=Url.SER_LOGIN%>" method="POST"><input type="hidden" name="clave" value="exit"/>
